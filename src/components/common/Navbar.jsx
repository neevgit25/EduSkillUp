...
//         console.log("Catalogs from bc:",subLinks)
//       } catch (error) {
   ...
   (
    async ()=>{
      ...
      try {
        ...
        console.log("Printing Sublinks result catalogs:", subLinks);
      } catch (error) {
    ...
    <li key={index}>
      {link.title === "Catalog" ? (  
        ...
        > 
          <p>{link.title}</p>
          <BsChevronDown />
         { console.log("inside sublinks",subLinks)}
          <div
          className="invisible absolute left-[50%] top-[50%] z-[1000] flex flex-col w-[200px] translate-x-[-50%] translate-y-[3em]
          rounded-md bg-richblack-5
          p-4 text-richblack-900 transition-all duration-150
          ...
          lg:w-[300px]
           select-none
          ">
            { console.log("inside sublinks  2",subLinks)}
          {/* // big rectangle */}
        <div
            className="absolute left-[50%] top-0
          translate-y-[-40%] translate-x-[80%] rotate-45
          -z-10 select-none
            h-6 w-6 rounded bg-richblack-5"></div>
              ...
              {loading?(<p className="text-center">Loading...</p>):subLinks?.length ? ( //non-empty
             <>
               { console.log("inside sublinks  3",subLinks)}
              {
              ...
              className="rounded-lg bg-transparent py-4 pl-4 hover:bg-richblack-50"
              >
                       { console.log("inside sublinks  4",subLinks)}
                <p className="">{subLink.name} </p>
              </Link>
            ))
              }
